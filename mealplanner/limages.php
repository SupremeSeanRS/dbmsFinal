...
<body>
    ...
    <?php 
        ...
        if (isset($_POST['upload'])) { 
            ...
            $mealid = "SELECT lunch.mealID FROM recipe JOIN lunch ON recipe.recID=lunch.recID WHERE recipe.recName='{$rname}' and lunch.uName='{$name}'";
            $mealquery = mysqli_query($conn, $mealid);
            while ($row = $mealquery->fetch_assoc()) {
                $mealidno = $row['mealID'];
            }
            
            //"SELECT DISTINCT lunch.mealID FROM lunch JOIN recipe ON lunch.recID=recipe.recID WHERE lunch.uName='{$name}'";
            ...
            if (isset($mealresult->num_rows) && $mealresult->num_rows > 0) {
                ...
                $sql = "INSERT INTO limages (mealID, uName,images) VALUES ('$mealidno','$name','$filename')";
                // Execute query
    ...
    <div class="container col-lg-5 mx-auto">
            ...
            <!-- First Name -->
            <div class="">
                <label class="" >Lunch Recipe Name:</label>
                <div class="">
