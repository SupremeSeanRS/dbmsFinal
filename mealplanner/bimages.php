...
<body>
    ...
    <?php 
        ...
        if (isset($_POST['upload'])) { 
            ...
            $mealid = "SELECT breakfast.mealID FROM recipe JOIN breakfast ON recipe.recID=breakfast.recID WHERE recipe.recName='{$rname}' and breakfast.uName='{$name}'";
            $mealquery = mysqli_query($conn, $mealid);
            while ($row = $mealquery->fetch_assoc()) {
                $mealidno = $row['mealID'];
            }
            
            //"SELECT DISTINCT breakfast.mealID FROM breakfast JOIN recipe ON breakfast.recID=recipe.recID WHERE breakfast.uName='{$name}'";
            ...
            if (isset($mealresult->num_rows) && $mealresult->num_rows > 0) {
                ...
                $sql = "INSERT INTO bimages (mealID, uName,images) VALUES ('$mealidno','$name','$filename')";
                // Execute query
    ...
    <div class="container col-lg-5 mx-auto">
            ...
            <!-- First Name -->
            <div class="">
                <label class="" >Breakfast Recipe Name:</label>
                <div class="">
